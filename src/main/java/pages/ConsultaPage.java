...
public class ConsultaPage {
	...
	private int diaDeseadoP = 0;
	private int mesDeseadoP = 0;
	private int añoDeseadoP = 0;
	...
	public void txtFechaPartida(String fechaPartida) {
		...
		String sMes = fecha[1];
		String sAño = fecha[2];
		diaDeseadoP = Integer.parseInt(sDia);
		mesDeseadoP = Integer.parseInt(sMes);
		añoDeseadoP = Integer.parseInt(sAño);
	...
	public void txtFechaRegreso(String fechaRegreso) throws InterruptedException {
		...
		String string1 = date1;
		String[] fechaAct = string1.split("/");
		String sDiaAct = fechaAct[0];
		int diaActual = Integer.parseInt(sDiaAct);
		String sMesAct = fechaAct[1];
		int mesActual = Integer.parseInt(sMesAct);
		String sAñoActual = fechaAct[2];
		int añoActual = Integer.parseInt(sAñoActual);
		...
		String añoR = fechaR[2];

		int diaDeseadoR = Integer.parseInt(diaR);
		int mesDeseadoR = Integer.parseInt(mesR);
		int añoDeseadoR = Integer.parseInt(añoR);
		
		if(añoDeseadoR >= añoDeseadoP) {
			if(mesDeseadoR >= mesDeseadoP) {
				if(diaDeseadoR >= diaDeseadoP) {
			...
			System.out.println("Fecha de Regreso no debe ser menor a Fecha de Salida");
		}
	...
	public void mensajeErrorFechaRegresoMenor() {
		System.out.println("Fecha de Regreso no debe ser menor a Fecha de Salida");
	}
